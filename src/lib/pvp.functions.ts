...
export const updatePlayerPvpSettings = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((data) => z.object({ pvp_mode: z.enum(['peaceful', 'manual', 'auto']) }).parse(data))
...
export const startPvpBattle = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((data) => z.object({ target_user_id: z.string().uuid() }).parse(data))
  .handler(async ({ data, context }) => {
    const supabase = context.supabase as any
    const userId = context.userId
    ...
    if (myParty) {
      const { data: targetInParty } = await supabase.from('party_members')
        .select('player_id').eq('party_id', myParty.party_id).eq('player_id', data.target_user_id).maybeSingle()
