...
const Panel = ({ title, children, accent = "#f5cf6b", style = {} }: any) => (
  <div style={{
    background: "rgba(11,5,16,0.92)", border: `1px solid ${accent}44`,
    borderRadius: 14, overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.4)", ...style
  }}>
    <div style={{ background: `linear-gradient(90deg, ${accent}22, transparent)`, padding: "8px 12px", borderBottom: `1px solid ${accent}22`, display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width: 4, height: 14, background: accent, borderRadius: 2 }} />
      <span style={{ fontSize: 11, fontWeight: 900, color: accent, letterSpacing: 1.5 }}>{title}</span>
    </div>
    <div style={{ padding: 12 }}>{children}</div>
  </div>
);

const GovernanteDialog = ({ onClose }: any) => {
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 999999, background: "rgba(0,0,0,0.85)", display: "grid", placeItems: "center", padding: 20 }}>
      <div style={{ width: "min(500px, 95%)", background: "#0b0510", border: "2px solid #f5cf6b", borderRadius: 20, padding: 24, textAlign: "center" }}>
        <div style={{ fontSize: 50, marginBottom: 15 }}>🏛️</div>
        <h2 style={{ color: "#f5cf6b", fontSize: 24, fontWeight: 900, marginBottom: 10 }}>GOVERNANTE DE RUBY</h2>
        <p style={{ color: "#eadfe8", lineHeight: 1.6, marginBottom: 20 }}>"Saudações, treinador. O portal para o Santuário Glacial está aberto apenas para aqueles que buscam um novo começo."</p>
        <button onClick={onClose} style={{ background: "#f5cf6b", color: "#0b0510", border: "none", borderRadius: 10, padding: "12px 30px", fontWeight: 800, cursor: "pointer" }}>FECHAR</button>
      </div>
    </div>,
    document.body
  );
...
const AnciaoGlacialDialog = ({ onReset, onClose }: any) => {
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 999999, background: "rgba(0,0,0,0.9)", display: "grid", placeItems: "center", padding: 20 }}>
      <div style={{ width: "min(600px, 95%)", background: "#060b14", border: "2px solid #6bd4ff", borderRadius: 24, padding: 30, textAlign: "center", boxShadow: "0 0 50px rgba(107,212,255,0.2)" }}>
        <div style={{ fontSize: 60, marginBottom: 15 }}>❄️</div>
        <h2 style={{ color: "#6bd4ff", fontSize: 28, fontWeight: 900, marginBottom: 10 }}>ANCIÃO GLACIAL</h2>
        <p style={{ color: "#bfe9ff", lineHeight: 1.7, marginBottom: 25 }}>"A geada purifica tudo. Você está pronto para sacrificar seu poder atual em troca de uma nova jornada e recompensas eternas?"</p>
        <div style={{ display: "flex", gap: 15, justifyContent: "center" }}>
           <button onClick={onClose} style={{ background: "transparent", border: "1px solid #6bd4ff", color: "#6bd4ff", borderRadius: 12, padding: "12px 24px", fontWeight: 700, cursor: "pointer" }}>AINDA NÃO</button>
           <button onClick={onReset} style={{ background: "#6bd4ff", color: "#060b14", border: "none", borderRadius: 12, padding: "12px 24px", fontWeight: 900, cursor: "pointer", boxShadow: "0 0 20px rgba(107,212,255,0.4)" }}>RECOMEÇAR JORNADA</button>
        </div>
      </div>
    </div>,
    document.body
