  ...
  return (
    <div style={{
      ...
      {worldMapOpen && createPortal((() => {
        ...
        const hasGov = (idle.items?.carta_governante ?? 0) > 0;
        return (
          <div onClick={() => setWorldMapOpen(false)} style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(0,0,0,0.9)", display: "grid", placeItems: "center", padding: 20, cursor: "pointer" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "#0b0510", border: "4px solid #f5cf6b", borderRadius: 20, padding: 24, maxWidth: 900, width: "100%", cursor: "default", boxShadow: "0 0 80px rgba(245,207,107,0.5)", position: "relative" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <h2 style={{ color: "#f5cf6b", margin: 0, fontSize: 24, fontWeight: 900 }}>🌏 MAPA MUNDI — CONT. {worldTab}</h2>
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => setWorldTab(t => t === 3 ? 1 : (t + 1) as 1|2|3)} style={{ background: "linear-gradient(135deg, #f5cf6b, #d9a441)", border: "none", color: "#160a20", borderRadius: 8, padding: "6px 12px", fontWeight: 900, cursor: "pointer" }}>TROCAR CONTINENTE</button>
                  <button onClick={() => setWorldMapOpen(false)} style={{ background: "#3a1010", border: "2px solid #f5cf6b", color: "#f5cf6b", borderRadius: 8, padding: "6px 12px", fontWeight: 900, cursor: "pointer" }}>✕</button>
                </div>
              </div>
              <div style={{ width: "100%", aspectRatio: "16/9", borderRadius: 12, overflow: "hidden", background: `url(${assetUrlFromJson(bg)}) center/cover`, position: "relative", border: "2px solid rgba(245,207,107,0.3)" }}>
                {PINS.map(pin => {
                  const m = IDLE_MAPS[pin.id]; if (!m) return null;
                  const isGovMap = pin.id === "absol_start" || pin.id === "governante_hall";
                  const locked = ((m.minLevel ?? 0) > (idle.trainerLevel ?? 1)) || (isGovMap && !hasGov);
                  return (
                    <div key={pin.id} onClick={() => { if (locked) return; setIdle(s => ({ ...s, currentMap: pin.id })); setWorldMapOpen(false); }}
                      style={{ position: "absolute", left: `${pin.x}%`, top: `${pin.y}%`, width: 24, height: 24, background: locked ? "#444" : "#f5cf6b", border: "2px solid #fff", borderRadius: "50%", cursor: locked ? "not-allowed" : "pointer", transform: "translate(-50%,-50%)", boxShadow: "0 0 10px #f5cf6b" }}
                    >
                      <div style={{ position: "absolute", top: 25, left: "50%", transform: "translateX(-50%)", background: "rgba(0,0,0,0.8)", padding: "2px 6px", borderRadius: 4, color: locked ? "#888" : "#f5cf6b", fontSize: 10, whiteSpace: "nowrap" }}>{m.name}</div>
                    </div>
                  );
                })}
              </div>
